...
try:
    _ROUGE_PATH = os.environ['ROUGE_HOME']
except KeyError:
    print('Warning: ROUGE is not configured')
    _ROUGE_PATH = None
...
def eval_rouge(dec_pattern, dec_dir, ref_pattern, ref_dir,
    ...
    """ evaluate by original Perl implementation"""
    # silence pyrouge logging
    assert _ROUGE_PATH is not None, "ROUGE_PATH is not set"
    log.get_global_console_logger().setLevel(logging.WARNING)
    with tempfile.TemporaryDirectory() as tmp_dir:
        Rouge155.convert_summaries_to_rouge_format(
            dec_dir, join(tmp_dir, 'dec'))
        ...
        cmd = (f'"{join(_ROUGE_PATH, "ROUGE-1.5.5.pl")}"'
               + f' -e "{join(_ROUGE_PATH, "data")}" '
               + cmd
               + f' -a "{join(tmp_dir, "settings.xml")}"')
...
def eval_meteor(dec_pattern, dec_dir, ref_pattern, ref_dir):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        cmd = 'java -Xmx80G -jar {} {} {} -l en -norm'.format(
