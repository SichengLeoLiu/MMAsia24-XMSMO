...
from my_model_noise_filter_topk_combined import CLIPSum
import codecs
import argparse
import torch
        ...
        for split in splits:
            ...
            for iteration, data in enumerate(data_loader):
                ...
                for idx, id in enumerate(file_id):
                    ...
                    dir = os.path.join('output_topk',dir1)
                ...
                if iteration%500==0:
                    checkpoint_path = os.path.join(save_model_path, "noise_filter_topk_combined_E%i-%9f.ckpt" % (iteration,tracker['LOSS'].mean()))
            ...
            if split == 'train' and epoch%5 ==0:
                checkpoint_path = os.path.join(save_model_path, "noise_filter_topk_combined_E%i-%9f.ckpt" % (epoch,tracker['LOSS'].mean()))
                torch.save(model.state_dict(), checkpoint_path)
                print("Model saved at %s" % checkpoint_path)
