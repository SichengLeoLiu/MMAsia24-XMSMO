...
def gumbel_softmax(logits, temperature, k, eps=1e-10):
    ...
    softmax = torch.softmax(logits_with_noise, dim=1)
...
class CLIPSum(nn.Module):
    def __init__(self, text_hidden_size, video_hidden_size, 
        ...
        self.linear_topk_video = nn.Linear(self.hidden_size, self.hidden_size)
        self.linear_topk_text = nn.Linear(self.hidden_size, self.hidden_size)
    ...
    def forward(self, input_text, input_video, text_summary_length=11):
        ...
        # print("image_feature", image_feature) # [2,60,512]
        image_feature=self.linear_topk_video(image_feature)
        word_feature=self.linear_topk_text(word_feature)
        # print("image_feature", image_feature) # [2,60,512]
        
        video_mutual_info, indices = gumbel_softmax(image_feature, self.temperature, image_feature.size(1) *2 // 3)
        text_mutual_info, indices = gumbel_softmax(word_feature, self.temperature, word_feature.size(1)*2 // 3)
        
        # ablation study(2)
        # video_mutual_info, indices = gumbel_softmax(image_feature, self.temperature, image_feature.size(1) *2 // 2)
        # text_mutual_info, indices = gumbel_softmax(word_feature, self.temperature, word_feature.size(1)*2 // 2)
        
        # video_mutual_info= image_feature
        # text_mutual_info=word_feature
        
        # print("video_mutual_info",video_mutual_info.shape)
        ...
        video_only_info_gate=self.filter_sigmoid(video_only_info_gate)
        image_feature=image_feature*video_only_info_gate
        ...
        text_only_info_gate=self.filter_sigmoid(text_only_info_gate)
        word_feature=word_feature*text_only_info_gate
        ...
        # ablation 3
        # word_feature = text_mutual_info

        # image_feature = video_mutual_info
        # text_b,text_s,text_d = word_feature.size()
        ...
        # word_feature=torch.cat([topic_distance_target.unsqueeze(1), word_feature], dim=1)
        ...
        text_logp = self.outputs2vocab(word_feature.reshape(-1, word_feature.size(2)))

        text_logp = text_logp.view(text_b, text_s, 1)
        # text_logp=text_logp[:,1:,:]
        # image_feature = self.image_pos_encoder(image_feature)

        # image_feature = torch.cat([topic_distance_target.unsqueeze(1), image_feature], dim=1)
        ...
        video_logp = self.outputs2coverframe(image_feature.reshape(-1, image_feature.size(2)))
        ...
        summary_image_feature = []
        for i, (image, summary) in enumerate(zip(scene_frame_pad_batch_list, video_logp)):
            ...
            summary_image_feature.append(image_feature[i][int(rank[0])])
            
        summary_image_feature = torch.stack(summary_image_feature)
    
        
        summary_sent_feature = []
        output_text_summaries = []
        output_text_summaries_pos = []

        for idx,(text, t_id, summary) in enumerate(zip(text_token_list, text_id_list, text_logp)):
            word_count = 0
            pos = []
            rank = torch.argsort(summary, dim=0, descending=True)
            ...
            for i in sorted(filtered_rank):
                ...
                pos.append(int(i))
                feature.append(word_feature[idx][int(i)])
                            # 检查feature列表中是否有空的张量
            non_empty_feature = [f for f in feature if f.numel() > 0]

            # 如果非空的张量数量大于0，则进行堆叠操作
            if non_empty_feature:
                feature = torch.stack(non_empty_feature)
            else:
                # 处理没有非空张量的情况，比如返回一个空的张量或者其他适当的操作
                feature = torch.zeros(512).unsqueeze(0).to("cuda")  # 返回一个空的张量示例，可以根据实际情况调整
            ...
            summary_sent_feature.append(feature)
            
        summary_sent_feature=pad_sequence(summary_sent_feature, batch_first=True, padding_value=0)
        ...
        return output_text_summaries, output_text_summaries_pos, text_logp, output_video_summaries, output_video_summaries_pos, video_logp, video_mutual_info, text_mutual_info, summary_image_feature, summary_sent_feature
