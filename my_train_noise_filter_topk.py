...
from my_model_noise_filter_topk import CLIPSum
import codecs
import argparse
import torch
...
class OT_topic():
    def __init__(self):
       ...
       # self.cliptext.eval()
        ...
        self.test_criterion =  SinkhornDistance(eps=0.1, max_iter=100, reduction=None).to('cuda')
    ...
    def score(self, summaries_f, bodies_f):
        ...
        for summ, bod in zip(summaries_f, bodies_f):
            ...
            # print(summ.shape)
            # print(bod.shape)
            summ = summ.reshape(2,256)
            bod = bod.reshape(2,256)
            score,_,_ = self.test_criterion(summ, bod)
            scores.append(score)
        ...
        for split in splits:
            ...
            for iteration, data in enumerate(data_loader):
                ...
                for idx, id in enumerate(file_id):
                    ...
                    dir = os.path.join('output_combined',dir1)
                ...
                if iteration%200==0:
                    checkpoint_path = os.path.join(save_model_path, "top_k_E%i-%9f.ckpt" % (iteration,tracker['LOSS'].mean()))
