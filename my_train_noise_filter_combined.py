...
from my_model_noise_filter_combined import CLIPSum
import codecs
import argparse
import torch
    ...
    for epoch in range(args.epochs):
        ...
        for split in splits:
            ...
            for iteration, data in enumerate(data_loader):
                ...
                if iteration%200==0:
                    checkpoint_path = os.path.join(save_model_path, "noise_filter_combined_E%i-%9f.ckpt" % (iteration,tracker['LOSS'].mean()))
            ...
            if split == 'train' and epoch%5 ==0:
                checkpoint_path = os.path.join(save_model_path, "noise_filter_combined_E_E%i-%9f.ckpt" % (epoch,tracker['LOSS'].mean()))
                torch.save(model.state_dict(), checkpoint_path)
                print("Model saved at %s" % checkpoint_path)
