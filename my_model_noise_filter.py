...
class CLIPSum(nn.Module):
    ...
    def forward(self, input_text, input_video, text_summary_length=11):
        ...
        # video_mutual_info, indices = gumbel_softmax(image_feature, self.temperature, image_feature.size(1) // 2)
        # text_mutual_info, indices = gumbel_softmax(word_feature, self.temperature, word_feature.size(1) // 2)
        ...
        for idx,(text, t_id, summary) in enumerate(zip(text_token_list, text_id_list, text_logp)):
            ...
            for i in rank:
                ...
                if word_count > self.max_summary_word:
                    break
            print("filtered_rank", filtered_rank)
